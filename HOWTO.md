...
You may modify at any time the credentials used to authenticate to connect modifying the file *pure-bg-tasks/.pure-bg-tasks_dev.env*.
...
```sh
$ docker compose up pure-bg-tasks_dev
```
...
```sh
$ docker compose run pure-bg-tasks_bash
```
...
```sh
$ docker compose run pure-bg-tasks_test
```
